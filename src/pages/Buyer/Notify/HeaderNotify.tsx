...
const HeaderNotify = () => {
  ...
  const { isLoading: isLoadingNotifications } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (response.status === 200) {
        setNotificationsAccount(response.data.data)
      }
  ...
  const { isLoading: isLoadingOrders } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (response.status === 200) {
        setNotificationsOrder(response.data.data)
      }
      return response.data.data
    },
  })
  ...
  return (
    <div className="dropdown dropdown-hover dropdown-bottom dropdown-end">
      <div tabIndex={0} role="button" className=" drawer-button text-gray-600 hover:text-black flex  items-center">
        ...
        Thông báo
        {notificationsAccount.filter((notification) => !notification.seen).length > 0 &&
          notificationsOrder.filter((notification) => !notification.seen).length > 0 && (
            <div className="badge badge-secondary text-white ml-2 badge-sm">
              {notificationsAccount.filter((notification) => !notification.seen).length +
                notificationsOrder.filter((notification) => !notification.seen).length}
            </div>
          )}
      </div>
            ...
            <div className="overflow-y-auto">
              {isLoadingNotifications || isLoadingOrders ? (
                ...
                <>
                  ...
                  {notificationsAccount.slice(0, 5).map((notification, index) => (
                    <div
                      key={index}
                  ...
                  {notificationsOrder.slice(0, 5).map((notification, index) => (
                    <div
                      key={index}
                      className={`mb-2 border-b grid grid-cols-10 items-center gap-2 ${
                        notification.seen ? 'opacity-70' : ''
